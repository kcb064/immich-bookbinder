...
- The per-IP limits use the client address the proxy reports. `TRUST_PROXY` (default `true`) says whose `X-Forwarded-For` to believe; if the container is also reachable without going through the tunnel or a reverse proxy, set it to `false` or to the proxy's IPs/CIDRs (`172.18.0.0/16,10.0.0.5`) so a direct client cannot pick its own address and dodge the limits.
