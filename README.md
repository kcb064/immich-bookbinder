...
> **Status: early development (M5).** You can connect Immich; start a book from an album, a detected trip (dates plus places), chosen people or a smart search; have every photo scored and de-duplicated; review why each one is in or out (and overrule it), with chapters by place and a per-person filter; get a chronological automatic layout with chapter opener spreads, face-aware crops and a wrap-around cover; tweak it in the editor (swap, remove, reorder, captions, chapter titles, templates) and on the book page (cover photo and text); check print readiness (page count, resolution, safety, cover, spine); download proof, 300 ppi print and cover PDFs (the cover sized by Lulu's own dimensions when connected); share the book through a public link (optional expiry, password and PDF download) that shows it as spreads without exposing Immich; and order hardcover copies from Lulu: the app publishes the PDFs for Lulu to fetch, runs Lulu's file validation, quotes the live price with shipping options, creates the print job (sandbox or production) and follows it to shipped with tracking links, while payment stays on lulu.com. The free-form designer is next (see the roadmap).
...
| M6 designer | Free-form slots, text boxes, snapping, undo/redo, keyboard navigation ([spec](docs/milestones/M6.md)) | planned |
...
Print geometry and PDF rules: [docs/print-specs.md](docs/print-specs.md). How photos are scored, de-duplicated and picked: [docs/selection.md](docs/selection.md). Lulu account and SKU notes: [docs/lulu-setup.md](docs/lulu-setup.md).
...
Root scripts call `corepack pnpm` internally, so they also work where `pnpm` itself is not on PATH. Milestone specs for the remaining roadmap rows live in [docs/milestones/](docs/milestones/README.md).
