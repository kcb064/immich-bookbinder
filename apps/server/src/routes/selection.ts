import { DecisionsInput, Id, SelectionRules, StartSelectionInput, targetPhotosFor, type SelectionView } from '@bookbinder/shared';
...
export const selectionRoutes: FastifyPluginAsync = async (app) => {
  ...
  function view(bookId: string, targetPages: number | undefined): SelectionView {
    ...
    const summary = app.candidates.summary(bookId, assets, targetPhotosFor(targetPages ?? 48));
    const run = app.selections.latest(bookId);
    return { candidates: ordered, ...(summary ? { summary } : {}), ...(run ? { run } : {}) };
  }
  ...
  app.get('/api/books/:id/selection', async (request, reply) => {
    ...
    return view(book.id, book.rules?.targetPages);
  });
